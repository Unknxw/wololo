 ...
 *
 * @param String $month Contient le mois à utiliser
 *
 * @return String le mois d'après
 */
function getMoisSuivant($month)
{
 ...
 */
function getMoisPrecedent($month)
{
    $numAnnee = substr($month, 0, 4);
    $numMois = substr($month, 4, 2);
    if ($numMois == '01') {
...
{
    global $monthDebut; // valeur dans majGSB.php
    $visitors = getLesVisiteurs($pdo);
    $monthActuel = getMois(date('d/m/Y'));
    $monthFin = getMoisPrecedent($monthActuel);
    foreach ($visitors as $unVisiteur) {
        $monthCourant = $monthFin;
        $idVisiteur = $unVisiteur['id'];
        $n = 1;
        while ($monthCourant >= $monthDebut) {
            if ($n == 1) {
                $etat = 'CR';
                $monthModif = $monthCourant;
            } else {
                if ($n == 2) {
                    $etat = 'VA';
                    $monthModif = getMoisSuivant($monthCourant);
                } else {
                    $etat = 'RB';
                    $monthModif = getMoisSuivant(getMoisSuivant($monthCourant));
                }
            }
            $numAnnee = substr($monthModif, 0, 4);
            $numMois = substr($monthModif, 4, 2);
            ...
            $req = 'insert into fichefrais(idvisiteur,mois,nbjustificatifs,'
                ...
                . "values ('$idVisiteur','$monthCourant',$nbJustificatifs,"
                . "0,'$dateModif','$etat');";
            $pdo->exec($req);
            $monthCourant = getMoisPrecedent($monthCourant);
            $n++;
        }
    }
}
    ...
    foreach ($lesFichesFrais as $uneFicheFrais) {
        ...
        foreach ($lesIdFraisForfait as $unIdFraisForfait) {
            ...
            $req = 'insert into lignefraisforfait(idvisiteur,mois,'
                ...
                . "values('$idVisiteur','$month','$idFraisForfait',$quantite);";
            $pdo->exec($req);
        }
    }
}
    ...
    foreach ($lesFichesFrais as $uneFicheFrais) {
        ...
        for ($i = 0; $i <= $nbFrais; $i++) {
            ...
            $req = 'insert into lignefraishorsforfait(idvisiteur,mois,libelle,'
                    . 'date,montant) '
                    . "values('$idVisiteur','$month','$lib','$hasardMois',"
                    . "$hasardMontant);";
            $pdo->exec($req);
        }
    }
...
{
    @list($jour, $month, $annee) = explode('/', $date);
    unset($jour);
    if (strlen($month) == 1) {
        $month = '0' . $month;
    }
...
function majFicheFrais($pdo)
{
    ...
    foreach ($lesFichesFrais as $uneFicheFrais) {
        ...
        $req = 'select sum(montant) as cumul from lignefraishorsforfait '
            ...
            . "and lignefraishorsforfait.mois = '$month' ";
        $res = $pdo->query($req);
        $ligne = $res->fetch();
        ...
        $req = 'select sum(lignefraisforfait.quantite * fraisforfait.montant) '
                ...
                . "and lignefraisforfait.mois = '$month' ";
        $res = $pdo->query($req);
        $ligne = $res->fetch();
        ...
        $req = "update fichefrais set montantvalide = $montantValide "
            . "where idvisiteur = '$idVisiteur' and mois = '$month'";
        $pdo->exec($req);
    }
}
