 ...
 * les 4 premiers pour la connection
 * $monPdo de type PDO
...
class PdoGsb
{
     ...
     * @param String $month Mois sous la forme aaaamm
     *
     * @return tous les champs des lignes de frais hors forfait sous la forme
     * d'un tableau associatif
     */
    public function getLesFraisHorsForfait($idVisiteur, $month)
    {
        $requetePrepare = PdoGsb::$monPdo->prepare(
        ...
        $requetePrepare->bindParam(':unMois', $month, PDO::PARAM_STR);
        $requetePrepare->execute();
        $lesLignes = $requetePrepare->fetchAll();
        for ($i = 0; $i < count($lesLignes); $i++) {
            $date = $lesLignes[$i]['date'];
            $lesLignes[$i]['date'] = toFrenchDate($date);
        }
        return $lesLignes;
    }
     ...
     */
    public function getNbjustificatifs($idVisiteur, $month)
    {
        $requetePrepare = PdoGsb::$monPdo->prepare(
     ...
     * associatif
     */
    public function getLesFraisForfait($idVisiteur, $month)
    {
        $requetePrepare = PdoGSB::$monPdo->prepare(
     ...
     * @return null
     */
    public function majFraisForfait($idVisiteur, $month, $lesFrais)
    {
     ...
     * un mois donné en enregistrant le nouveau hors forfait
     *
     * @param String $idVisiteur ID du visiteur
     * @param String $month Mois sous la forme aaaamm
     * @param Array $libelle Nom du libelle
     * @param Date $date La date actuelle
     * @param Float $montant Le montant du frais à ajouter      *
     * @param int $idFrais L'id du frais à modifier
     * @return null
     */
    public function majFraisHorsForfait($idVisiteur, $month, $libelle, $date, $montant, $idFrais)
    {
        $dateAnglaise = toDate($date);
        ...
        $requetePrepare->bindParam(':uneDate', $dateAnglaise, PDO::PARAM_STR);
     ...
     * @return null
     */
    public function majNbJustificatifs($idVisiteur, $month, $nbJustificatifs)
    {
        $requetePrepare = PdoGSB::$monPdo->prepare(
            'UPDATE fichefrais '
     ...
     * @return vrai ou faux
     */
    public function estPremierFraisMois($idVisiteur, $month)
    {
        $boolReturn = false;
        $requetePrepare = PdoGsb::$monPdo->prepare(
     ...
     * @return null
     */
    public function creeNouvellesLignesFrais($idVisiteur, $month)
    {
     ...
     *         et la ligne d'état
     */
    public function getLesInfosFicheFrais($idVisiteur, $month)
    {
        $requetePrepare = PdoGSB::$monPdo->prepare(
     ...
     * @return null
     */
    public function majEtatFicheFrais($idVisiteur, $month, $etat)
    {
        $requetePrepare = PdoGSB::$monPdo->prepare(
            'UPDATE ficheFrais '
    ...
    public function getAllVisiteur()
    {
        ...
        $requetePrepare->execute();
        $visitors = $requetePrepare->fetchAll();
        return $visitors;
    }

    /**
     * Retourne les mois pour lesquel un visiteur a une fiche de frais validé ou remboursé
     *
     * @param String $idVisiteur ID du visiteur
     *
     * @return un tableau de mois
     */
    public function getLesMoisPaiement($idVisiteur)
    {
        $requetePrepare = PdoGSB::$monPdo->prepare(
            'SELECT fichefrais.mois AS mois FROM fichefrais '
            . 'WHERE fichefrais.idvisiteur = :unIdVisiteur '
    ...
    /*
     * Calcul le montant total d'une fiche et ajoute le total dans la base de donnée
     */
    public function calculTotalFrais($idVisiteur, $month, $lesFrais, $lesFraisHorsForfait)
    {
