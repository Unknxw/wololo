 ...
 * @return null
 */
function connect($idVisiteur, $nom, $prenom, $type)
{
 ...
 */
function toDate($maDate)
{
    @list($jour, $month, $annee) = explode('/', $maDate);
    return date('Y-m-d', mktime(0, 0, 0, $month, $jour, $annee));
}
 ...
 */
function toFrenchDate($maDate)
{
    @list($annee, $month, $jour) = explode('-', $maDate);
 ...
 * @return Boolean vrai ou faux
 */
function isArray($tabEntiers)
{
    $boolReturn = true;
    foreach ($tabEntiers as $unEntier) {
        if (!isPositif($unEntier)) {
            $boolReturn = false;
        }
    }
    return $boolReturn;
...
function estDateDepassee($dateTestee)
{
    ...
    @list($jour, $month, $annee) = explode('/', $dateActuelle);
    $annee--;
    $anPasse = $annee . $month . $jour;
    @list($jourTeste, $monthTeste, $anneeTeste) = explode('/', $dateTestee);
    return ($anneeTeste . $monthTeste . $jourTeste < $anPasse);
}
